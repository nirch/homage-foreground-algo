...
int	CUniformBackground::Process( image_type *sim, int iFrame, image_type **cim )
{
	...
	gpTime_start( &m_tGuasian );
	image1_convolution_guasian( m_cim, 0.75, m_cim );
	gpTime_stop( &m_tGuasian );
	*cim = m_cim;
...
void CUniformBackground::Trace( FILE *fp )
{
	...
	gpTime_print( fp, "NN", &m_tNN );
	gpTime_print( fp, "Guasian", &m_tGuasian );
...
char * CUniformBackground::GetProcessLog()
{
	...
	sprintf( m_processLog, "%d  %d  %d %d", gpTime_mpf( &m_tCompare), gpTime_mpf( &m_tNN),  gpTime_mpf( &m_tGuasian), gpTime_mpf( &m_tUpdate) );
...
void CUniformBackground::ProcessLog()
{
	...
	int nn = gpTime_mpf( &m_tNN);
	int g = gpTime_mpf( &m_tGuasian);
