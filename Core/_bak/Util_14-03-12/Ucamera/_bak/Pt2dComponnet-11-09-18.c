...
static int	choose_group( pt2dGroupA_type *ag, int T );
...
static void	ln_force_smooth( ln_type *link );
...
#ifdef _AA_
treeGroup_type *
triangleA_group_componnet( pt2dA_type *apt, pt2dGroup_type *g )
{
pt2dA_type *aptc;
contour_type	*c;
	...
	aptc = pt2dA_copy_group( apt, g->group, g->no );

	c = pt2dA_bounding_contour( aptc );

	pt2dA_destroy( aptc );



	treeGroup_type *tg = ( treeGroup_type *)malloc( sizeof(treeGroup_type) );
	tg->ctr = g->p;
	tg->tree = g->no;

	tg->c = c;

	tg->area = contour_area( tg->c );
...
float	a;
	...
	u.x = l0->u.x * -link->u.y + l0->u.y *link->u.x;
	u.y = l0->u.x * link->u.x + l0->u.y *link->u.y;

	a = -u.x *link->len / ( 4* u.y);

	if( ABS(a) > 0.25*link->len )
		a = -0.25*link->len;
...
static int
choose_group( pt2dGroupA_type *ag, int T )
{
int	i,	imax,	no;
