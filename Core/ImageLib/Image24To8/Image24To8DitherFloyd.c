...
image_type *
image_24to8_dithering_floyd(image_type * im, palette_type * palette, box2i * pBox)
{
image_type * result;
	...
	case PALETTE_HALFTONE:
		result = image_24to8_HALFTONE_dithering_floyd(
							im, palette, pBox);
		break;

	case PALETTE_222:
		result = image_24to8_222_dithering_floyd(
							im, palette, pBox);
		break;

	case PALETTE_GRAY:
		result = image_24to8_gray_dithering_floyd(im, palette, pBox);
		break;

	case PALETTE_SMART:
		result = image_24to8_smart_dithering_floyd(
							im, palette, pBox);
		break;
	...
	case PALETTE_UNKNOWN:
		result = image_24to8_UNKNOWN_dithering_floyd(
							im, palette, pBox);
		break;
...
image_type *
image_24to8_UNKNOWN_dithering_floyd(image_type * im, palette_type * palette, box2i * pBox)
{
...
image_type *
image_24to8_smart_dithering_floyd(image_type * im, palette_type * palette, box2i * pBox)
{
...
char order;
	...
	for( i = top; i <= bottom; i++, order = !order){
		...
		for (j = left; j <= right; j++){
			...
			k = ((((r_set >> sh1) << PL_NBITS) + (g_set >> sh1)) << PL_NBITS) + 
									(b_set >> sh1);
			*tp = palette->table[k];
...
image_type * 
image_24to8_222_dithering_floyd(image_type * im, palette_type * p, box2i * pBox)
{
image_type *result;
int bottom, top, left, right;
int row, column, column2;
...
image_type * 
image_24to8_gray_dithering_floyd(image_type * im, palette_type * p, box2i * pBox)
{
image_type *result;
int bottom, top, left, right;
int row, column, column2;
...
char order;
	...
	for( i = top; i <= bottom; i++, order = !order){
		...
		for (j = left; j <= right; j++){
			...
			y  =  0.29900 * r_set + 0.58700 * g_set + 0.11400 * b_set + 0.5;

			*tp = colors_index[y];

			r_error = r_sum - p->data[*tp].Red;  //r_real
			g_error = g_sum - p->data[*tp].Green;//g_real
			b_error = b_sum - p->data[*tp].Blue; //b_real
...
image_type * 
image_24to8_HALFTONE_dithering_floyd(image_type * im, palette_type * p, box2i * pBox)
{
image_type *result;
int bottom, top, left, right;
int row, column, column2;
...
image_type * 
image_24to8_BW_dithering_floyd(image_type * im, palette_type * p, box2i * pBox)
{
image_type *result;
...
image_type * 
image_24to8_WB_dithering_floyd(image_type * im, palette_type * p, box2i * pBox)
{
image_type *result;
int bottom, top, left, right, row, column;
u_char	*tp;
