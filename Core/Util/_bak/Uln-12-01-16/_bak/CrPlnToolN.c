...
int
crPln_createN( pln_type *bpl, pln_type *pl, int iPl, float r1, float r2, crPln_type cr[] )
{
vec2f_type	ctr,	v;
ln_type	*l;
float	t,	gt;
...
float	T;
	...
	for( t = 0, gt = 0, l = pl->link, ctr = pl->ctr ; gt < pl->len ; t += dt, gt += dt ){
		...
		d = &ad[nD++];
		if( ln_distance( &bpl->ctr, bpl->link, NULL, &v, &d->l, &d->u, &d->t, &d->gt ) < 0 ||  d->gt < 0 || d->gt > bpl->len || ABS(d->u) > r1 ){
			d->l = NULL;
			d->u = 1000;
		}
		else
