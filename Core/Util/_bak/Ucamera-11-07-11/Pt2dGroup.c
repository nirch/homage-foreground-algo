...
int	nGroup;
	...
	for( i = 1; i < apt->nP; i++ ){
		...
		for( j = i-1 ; j >= 0 ; j-- ){
			...
			if( mask & ( ((_int64)1)<< cpt->group)  )	continue;
		...
		if( mask == 0 ){
			...
			for( k = 0 ; k < nGroup ; k++ )
				if( (mGroup & ( ((_int64)1) <<k)) == 0 )
					break;
			pt->group = k;
			if( k >= nGroup )	nGroup++;
			mGroup |= ( ((_int64)1)<<pt->group);
			continue;
		}
		...
		for( j = 0 ; j < i ; j++ ){
			pt2d_type	*cpt = &apt->p[j];
			if( mask & ( ((_int64)1)<< cpt->group)  )
				cpt->group = pt->group;

			mGroup |= ( ((_int64)1)<< cpt->group);
		}
	}
}
