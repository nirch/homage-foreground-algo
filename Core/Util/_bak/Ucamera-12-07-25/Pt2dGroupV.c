...
pt2dA_group_v( pt2dA_type *apt, float d )
{
	...
	for( i = 0 ; i < N ; i++ ){
		...
		if( bi[i0] >= bi[i] )	continue;

		i1  = ( i < N-1)? i+1 : 0;

		if( bi[i1] > bi[i] )	continue;

		i2 = -1;
		if( bi[i1] == bi[i] ){
			i2 = i1;
			 i2  = ( i1 < N-1)? i1+1 : 0;
			 if( bi[i2] >= bi[i] )	continue;
		}
		...
		if( i2 >= 0 )
		pt2dA_group_set_id( apt, apt->nP, i2, i );
	}
...
pt2dGroupA_type *
pt2dA_group_v2( pt2dA_type *apt, float d )
{
...
pt2dA_group_v1( pt2dA_type *apt, float d )
{
	...
	for( i = 0 ; i < 180 ; i++ ){
		int i0 = ( i > 0 )?i-1 : 180-1;
		int i1  = ( i < 180-1)? i+1 : 0;
